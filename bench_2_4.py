...
def benchmark_quant_24(A, B, meta, C, s, thread_k, thread_m, sms):
    workspace = torch.zeros(
        C.shape[1] // 128 * 16, device=torch.device("cuda:0"), dtype=torch.int32
    )
    # print("A:", A.shape, "B:", B.shape, "meta:", meta.shape, "C:", C.shape, "s:", s.shape)
    res = benchmark(
        lambda: marlin.mul_2_4(A, B, meta, C, s, workspace, thread_k, thread_m, sms)
    )
    return {
        "s": res,
        "TFLOP/s": 2 * A.numel() * C.shape[1] / res / 10**12,
        "GB/s": (2 * A.numel() + 4 * B.numel() * 2 + 2 * C.numel() + 2 * s.numel())
        / res
        / 10**9,
    }
...
for groupsize in [-1, 128] if ALL else [128]:
    ...
    for model, layers in MODELS.items():
        ...
        for batch in batchsizes:
            ...
            print(
                "[NN]  batch=%04d: s=%.5f, TFLOP/s=%07.3f, GB/s=%08.3f, speedup=%.2f"
                % (batch, tot_q["s"], tot_q["TFLOP/s"], tot_q["GB/s"], tot_q["speedup"])
            )
            print(
                "[2:4] batch=%04d: s=%.5f, TFLOP/s=%07.3f, GB/s=%08.3f, speedup=%.2f"
                % (
                    batch,
                    tot_q_24["s"],
                    tot_q_24["TFLOP/s"],
